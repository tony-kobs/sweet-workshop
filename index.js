...
`,n.querySelector(`.order-btn`).addEventListener(`click`,()=>{i(),ge(e)})}catch{n.innerHTML=`
  ...
  `).join(``);K.insertAdjacentHTML(`beforeend`,t)}function be(e){J.innerHTML=[`<label class="category-btn">
      ...
      `}).join(``)}function Oe(e){let t=e.activeIndex>(e.previousIndex??0)?`.feedback-arrow-next, .feedback-mobile-arrow-next`:`.feedback-arrow-prev, .feedback-mobile-arrow-prev`,n=document.querySelectorAll(t);n.forEach(e=>e.classList.add(`is-active`)),setTimeout(()=>{n.forEach(e=>e.classList.remove(`is-active`))},200)}function ke(){b(Z),Z=null,Z=le(`.feedback-slider-container`,{slideChange:Oe})}function Ae(){X&&(X.innerHTML=``);let e=document.querySelector(`.feedback-pagination`);e&&(e.innerHTML=``)}function $(e){return e?.response?.status??e?.status??e?.code??`Network Error`}async function je(){E(),Ae();try{let e=Ee(await se({page:Te,limit:we}));if(!e.length){D();return}X.innerHTML=De(e),requestAnimationFrame(()=>{ke(),D()})}catch(e){A(`Не вдалося завантажити відгуки клієнтів. Спробуйте пізніше. (${$(e)})`),D()}}je();var Me=e(te(),1);function Ne(){let e=document.getElementById(`faqAccordion`);if(!e)return console.warn(`FAQ акордеон: контейнер #faqAccordion не знайдено`),null;document.querySelectorAll(`.ac-trigger`).forEach(e=>{e.querySelector(`.faq-icon`)||e.insertAdjacentHTML(`beforeend`,`
